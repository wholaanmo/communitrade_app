...
import { postsDB } from '@/lib/firebaseHelpers'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribeRequests = postsDB.listenToPosts(
      ...
      (requestsData) => {
        ...
        setStats(prev => ({
          ...prev,
          myRequests: requestsData.filter(req => req.userId === session.user.uid).length
        }));
      }
    );
    ...
    const unsubscribeOffers = postsDB.listenToPosts(
      ...
      (offersData) => {
        ...
        setStats(prev => ({
          ...prev,
          myOffers: offersData.filter(offer => offer.userId === session.user.uid).length
        }));
      }
    );
  ...
  const formatPostForDisplay = (post, type) => {
    if (type === 'request') {
      return {
        ...
        timePosted: post.createdAt ? formatTimeAgo(post.createdAt) : 'Recently',
        request: post.title || post.request || 'Untitled Request',
        description: post.description || 'No description provided',
        category: post.category || 'General',
        location: post.location || 'Not specified',
        skillsOffered: Array.isArray(post.skillsOffered) ? post.skillsOffered : 
                      (post.skillsOffered ? [post.skillsOffered] : ['Not specified']),
      ...
      return {
        ...
        skills: Array.isArray(post.skills) ? post.skills : 
                (post.skills ? [post.skills] : ['Not specified']),
        description: post.description || 'No description provided',
        category: post.category || 'General',
        location: post.location || 'Not specified',
        skillsRequested: Array.isArray(post.skillsRequested) ? post.skillsRequested : 
                        (post.skillsRequested ? [post.skillsRequested] : ['Not specified']),
        schedule: post.schedule || 'Flexible',
        additionalNotes: post.additionalNotes || '',
        userId: post.userId,
        userEmail: post.userEmail
  ...
  const handleReportSubmit = async (reportData) => {
    ...
    try {
      ...
      await postsDB.reportPost(
        currentReportPost.request ? 'requests' : 'offers',
  ...
  const openProfileModal = async (post) => {
    try {
      // In a real app, you would fetch the user profile from Firebase
      // For now, we'll create a mock profile based on the post
      const mockProfile = {
  ...
  const handleContact = async (post) => {
    try {
      // In a real app, you would create a message in Firebase
      console.log('Contacting:', post.userName);
      alert(`Opening chat with ${post.userName}`);
    } catch (error) {
  ...
  const filteredRequests = useMemo(() => {
    if (!searchQuery) return requests.slice(0, 3).map(req => formatPostForDisplay(req, 'request'));
  ...
  const filteredOffers = useMemo(() => {
    if (!searchQuery) return offers.slice(0, 3).map(offer => formatPostForDisplay(offer, 'offer'));
