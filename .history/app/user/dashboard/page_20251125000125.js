  ...
  <div className="flex flex-col space-y-6 max-h-[80vh] overflow-y-auto">
    ...
    <div className="report-reasons flex flex-col gap-3">
      {reportReasons.map((reason) => (
        <label
          key={reason.value}
          className={`flex items-center p-3 border border-gray-300 rounded-lg cursor-pointer transition-all duration-300 hover:border-[#121731] hover:bg-gray-50`}
        >
          <Input
            type="radio"
            name="report-reason"
    ...
    {selectedReason === 'other' && (
      <div className="description-box flex flex-col gap-2">
        ...
        <TextArea
          ...
          rows={4}
          className="border border-gray-300 rounded-lg p-2 focus:outline-none focus:border-[#121731]"
        />
      </div>
    )}

    {/* Proof Upload */}
    <div className="proof-upload-section flex flex-col gap-2">
      <Label htmlFor="proof-upload">Proof (Optional)</Label>
      <FileUpload
        id="proof-upload"
        onChange={handleProofUpload}
        accept="image/*,.pdf,.doc,.docx"
        className="border border-gray-300 rounded-lg p-2 cursor-pointer hover:border-[#121731]"
      >
    ...
    {/* Action Buttons */}
    <div className="report-actions flex gap-3 justify-end mt-4">
      <Button
        variant="secondary"
