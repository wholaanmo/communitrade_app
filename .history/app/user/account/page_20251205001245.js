...
export default function AccountPage() {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        if (firebaseUser) {
          // Use data from Firebase if it exists
          setUser(prev => ({
            ...prev,
          ...
          // Set profile image from Firebase or session
          ...
          // No data in Firebase, use session data
          setUser(prev => ({
            ...prev,
  ...
  const toggleEditMode = () => {
    ...
    if (!isEditing) {
      ...
      setEditData({
        ...
        email: user.email, // Email is included but will be disabled
        phone: user.phone,
  ...
  const saveChanges = () => {
    const updatedUser = {
      ...
      email: editData.email, // Email is kept from original, not editable
      phone: editData.phone,
      governmentId: editData.governmentId // This was missing!
    };
    ...
    if (editData.idPhoto) {
      ...
      // Save ID photo URL to Firebase
      saveUserToFirebase({ 
        idPhotoUrl: newIdPhotoUrl,
        ...updatedUser // Include all updates including governmentId
      });
    } else {
      // Save all changes to Firebase (including governmentId)
  ...
  const cancelEdit = () => {
    ...
    // (Profile picture is saved immediately, so no need to reset)
  }
  ...
  // Update profile picture to use session image or uploaded image
