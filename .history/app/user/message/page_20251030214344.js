...
export default function MessagePage() {
  ...
  return (
    <div className="message-page flex h-[calc(100vh-60px)] -mt-[3px] bg-[#ffffff]">
      {/* Sidebar with conversation list */}
      <div className={`conversation-sidebar bg-white border-r border-[#728a9c] flex flex-col transition-all duration-300 overflow-hidden ${!isSidebarExpanded ? 'sidebar-collapsed w-[60px]' : 'w-[350px]'}`}>
        ...
        <div className="conversation-list flex-1 overflow-y-auto pr-1">
          {conversations.map((conversation) => (
            <div
              key={conversation.id}
              className={`conversation-item flex items-center p-4 cursor-pointer border-b border-[#728a9c] transition-colors duration-200 relative ${
            ...
            >
              ...
              <div className="conversation-info flex-1 min-w-0">
                <div className={`user-name font-semibold mb-1 ${
                  activeConversation?.id === conversation.id ? 'text-white' : 'text-[#121731]'
                }`}>
                  {conversation.userName}
                </div>
                <div className={`last-message text-sm ${
          ...
          <div className="chat-container flex flex-col h-full w-[103%]">
            ...
            >
              {activeConversation.messages.map((message) => (
                <div
                  key={message.id}
                  className={`message flex flex-col max-w-[70%] ${
                    message.sender === 'me' 
                      ? 'self-end items-end' 
                      : 'self-start items-start'
                  }`}
                >
                  <div className={`message-content px-4 py-3 rounded-2xl break-words ${
                    message.sender === 'me'
                    ...
                    {message.type === 'image' ? (
                      <img 
                        src={message.content} 
                        alt="Sent image" 
                        className="message-image max-w-[300px] max-h-[300px] rounded-lg cursor-pointer transition-transform duration-200 hover:scale-102"
            ...
            <div className="message-input-container p-4 border-t border-[#728a9c] bg-white">
              <div className="input-wrapper flex gap-2 items-center">
                ...
                <button 
                  ...
                  className="attachment-button bg-none border-none text-[#121731] cursor-pointer p-2 rounded-full transition-colors duration-200 hover:bg-[#ffffff]"
                >
                ...
                <button 
                  onClick={sendMessage} 
                  className="send-button bg-[#121731] text-white border-none rounded-full w-10 h-10 flex items-center justify-center cursor-pointer transition-colors duration-200 hover:bg-[#555555]"
                >
      ...
      {selectedImage && (
        <div 
          className="image-modal fixed inset-0 bg-black/90 flex items-center justify-center z-[1000]"
        ...
        >
          <div className="modal-content relative max-w-[90%] max-h-[90%]">
            ...
            />
            <button 
              className="close-modal absolute -top-10 right-0 bg-none border-none text-white text-2xl cursor-pointer p-2"
              onClick={closeImageModal}
            >
