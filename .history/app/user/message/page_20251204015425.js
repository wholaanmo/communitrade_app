...
export default function MessagePage() {
    ...
    const unsubscribe = messagingDB.listenToUserConversations(
      ...
      (firebaseConversations) => {
        ...
        const formattedConversations = firebaseConversations.map(conv => ({
          id: conv.id, // Make sure this is the Firebase key
          firebaseId: conv.id, // Store as firebaseId too
  ...
  useEffect(() => {
    ...
    const handleUrlParams = async () => {
      try {
        ...
        const formattedConversation = {
          id: conversation.id, // This should be the Firebase key
          firebaseId: conversation.id, // Also store as firebaseId for clarity
        ...
        // Check if conversation already exists in list and deduplicate
        setConversations(prev => {
          // Remove any existing conversation with the same ID
          const filteredPrev = prev.filter(conv => conv.id !== formattedConversation.id);
          // Add the new conversation and deduplicate
          const newConversations = [formattedConversation, ...filteredPrev];
          return removeDuplicateConversations(newConversations);
        });
    ...
    handleUrlParams();
  }, [userIdFromUrl, userNameFromUrl, userEmailFromUrl, userPictureFromUrl, isMobile, conversations, isInitializing, getCurrentUserId, session]);
  ...
  const removeDuplicateConversations = (conversations) => {
    ...
    return conversations.filter(conv => {
      if (seen.has(conv.id)) {
        console.warn('Removing duplicate conversation with id:', conv.id);
        return false;
      }
      seen.add(conv.id);
      return true;
        ...
        <div className="conversation-list flex-1 overflow-y-auto pr-1">
          {conversations.length === 0 ? (
            ...
            conversations.map((conversation, index) => (
              <div
                key={`conv-${conversation.id}-${index}`} // Add index as fallback
  ...
  <div
    key={`${message.id}-${index}`} // Add index as fallback
