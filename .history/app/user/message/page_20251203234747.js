...
export default function MessagePage() {
  ...
  // Function to find or create a conversation - FIXED: removed conversations dependency
  const findOrCreateConversation = useCallback((userId, userName, userEmail) => {
  ...
  // Effect to handle URL parameters - FIXED: only runs when params change
  useEffect(() => {
    if (!userIdFromUrl || !userNameFromUrl || isInitializing) return;

    // Find or create conversation based on URL parameters
    let conversation = conversations.find(conv => conv.userId === userIdFromUrl);
  ...
  }, [userIdFromUrl, userNameFromUrl, userEmailFromUrl, isMobile, conversations, isInitializing]);
  ...
  }, [conversations, isMobile, userIdFromUrl, userNameFromUrl, router]);
