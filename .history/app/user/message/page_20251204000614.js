...
export default function MessagePage() {
    ...
    const unsubscribe = messagingDB.listenToUserConversations(
      ...
      (firebaseConversations) => {
        ...
        const formattedConversations = firebaseConversations.map(conv => ({
          ...
          unreadCount: conv.unreadCount?.[getCurrentUserId()] || 0,
          status: 'online', // You can add online status tracking later
          firebaseData: conv
        }));
  ...
  useEffect(() => {
    if (!userIdFromUrl || !userNameFromUrl || isInitializing || !getCurrentUserId()) return;
    ...
    const handleUrlParams = async () => {
      try {
        ...
        const formattedConversation = {
          ...
          unreadCount: conversation.unreadCount?.[getCurrentUserId()] || 0,
          status: 'online',
    ...
    handleUrlParams();
  }, [userIdFromUrl, userNameFromUrl, userEmailFromUrl, isMobile, conversations, isInitializing, getCurrentUserId]);
  ...
  }, [conversations, isMobile, userIdFromUrl, userNameFromUrl, router, getCurrentUserId]);
