...
export default function MessagePage() {
  ...
  const [hasProcessedUrlParams, setHasProcessedUrlParams] = useState(false); // Add this line
  ...
  // Define removeDuplicatesById here, before any useEffect that uses it
  const removeDuplicatesById = useCallback((array) => {
    const seen = new Set();
    const result = [];
    ...
    for (const item of array) {
      if (!seen.has(item.id)) {
        ...
        console.warn('Found duplicate ID in removeDuplicatesById:', item.id);
      }
    }
    
    console.log(`Removed ${array.length - result.length} duplicates from array`);
    return result;
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubscribe = messagingDB.listenToUserConversations(
      ...
      (firebaseConversations) => {
        ...
        console.log('Firebase conversation IDs:', firebaseConversations.map(c => c.id));
        ...
        const uniqueConversations = removeDuplicatesById(formattedConversations);
        console.log('Unique conversations after deduplication:', uniqueConversations.map(c => c.id));
        
        // Use functional update to merge with any existing conversations
        setConversations(prev => {
          // Combine previous and new conversations, then remove duplicates
          const combined = [...uniqueConversations, ...prev];
  ...
  }, [session, getCurrentUserId, removeDuplicatesById]);
  ...
  useEffect(() => {
    if (!userIdFromUrl || !userNameFromUrl || isInitializing || !getCurrentUserId() || !session || hasProcessedUrlParams) return;
    ...
    const handleUrlParams = async () => {
      ...
      try {
        ...
        console.log('Calling getOrCreateConversation with:', {
          currentUserId: getCurrentUserId(),
        ...
        console.log('Received conversation:', conversation.id);
        ...
        setConversations(prev => {
          ...
          const existingIndex = prev.findIndex(c => c.id === formattedConversation.id);
          
          if (existingIndex === -1) {
            // Add to beginning of list and remove any duplicates
            const newList = [formattedConversation, ...prev];
            return removeDuplicatesById(newList);
          } else {
            // Update existing conversation
            const updated = [...prev];
            updated[existingIndex] = formattedConversation;
            return removeDuplicatesById(updated);
          }
        });
        
        // Select this conversation
        console.log('Selecting conversation:', formattedConversation.id);
    ...
    handleUrlParams();
  }, [userIdFromUrl, userNameFromUrl, userEmailFromUrl, userPictureFromUrl, isMobile, isInitializing, getCurrentUserId, session, hasProcessedUrlParams, removeDuplicatesById, selectConversation, router]);
        ...
        <div className="conversation-list flex-1 overflow-y-auto pr-1">
          {conversations.length === 0 ? (
            ...
            conversations.map((conversation, index) => {
              // Log each conversation being rendered
              console.log(`Rendering conversation ${index}:`, conversation.id);
              return (
                <div
                  key={conversation.id}
                ...
                >
                  <div className="avatar mr-3 sm:mr-4 flex-shrink-0">
                    <img 
                      ...
                      onError={(e) => {
                        e.target.src = 'data:image/svg+xml;base64,';
                      }}
                    />
                  </div>
