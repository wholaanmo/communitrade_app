  ...
  useEffect(() => {
    ...
    const handleUrlParams = async () => {
      ...
      try {
        ...
        // Log current conversations before update
        console.log('Current conversations before update:', conversations.map(c => c.id));
        
        // Check if conversation already exists in list
        const existingIndex = conversations.findIndex(c => c.id === formattedConversation.id);
        console.log('Existing index:', existingIndex, 'for conversation ID:', formattedConversation.id);
        
        if (existingIndex === -1) {
          // Add to beginning of list
          console.log('Adding new conversation to list:', formattedConversation.id);
          setConversations(prev => {
            const newConversations = [formattedConversation, ...prev];
            console.log('New conversations list after addition:', newConversations.map(c => c.id));
          ...
          setConversations(prev => {
            ...
            console.log('Updated conversations list:', updated.map(c => c.id));
            return updated;
          });
        }
    ...
    handleUrlParams();
  }, [userIdFromUrl, userNameFromUrl, userEmailFromUrl, userPictureFromUrl, isMobile, conversations, isInitializing, getCurrentUserId, session, hasProcessedUrlParams]);
