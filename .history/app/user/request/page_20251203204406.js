...
// Wrapper component for PostCard with custom buttons
// Wrapper component for PostCard with custom buttons - Improved version
const PostCardWithButtons = ({ 
  ...
  return (
    <div className="relative">
      {/* Wrap PostCard in a container that includes the buttons */}
      <div className="relative bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow duration-300">
        <PostCard
          post={post}
          {...props}
          // Disable the built-in buttons
          showDefaultActions={false}
        />
        {/* Buttons positioned at the bottom right inside the card */}
        <div className="absolute bottom-4 right-4 flex gap-2">
          {isOwnPost ? (
            // Show Delete and Edit for own posts
            <>
              <RequestActionButton 
                type="delete" 
            ...
            // Show Report, Contact, View Profile for others' posts
            <>
              <RequestActionButton type="report" onClick={() => onReport(post)} className="text-xs shadow-sm">
                Report
              </RequestActionButton>
              <RequestActionButton type="contact" onClick={() => onContact(post)} className="text-xs shadow-sm">
                Contact
              </RequestActionButton>
              <RequestActionButton type="profile" onClick={() => onViewProfile(post)} className="text-xs shadow-sm">
                View Profile
              </RequestActionButton>
            </>
          )}
...
export default function SkillRequestPlatform() {
  ...
  useEffect(() => {
    ...
    // Extract user ID EXACTLY as stored in database
    let userId = null;
    ...
    // Add real-time listener for ALL posts
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      if (editingPostId) {
        ...
        await postsDB.updatePost('requests', editingPostId, postData);
  ...
  const handleDelete = async (post) => {
    ...
    try {
      await postsDB.deletePost('requests', post.id);
