...
useEffect(() => {
  ...
  const loadPosts = async () => {
    setLoading(true);
    try {
      console.log('Calling postsDB.getPosts...');
  ...
  const postsRef = ref(db, 'requests');
  const unsubscribe = onValue(postsRef, (snapshot) => {
    ...
    if (snapshot.exists()) {
      ...
      console.log('Real-time posts:', posts);
      ...
      console.log('No data in real-time snapshot');
      setPosts([]);
    }
  });
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      const postData = {
        ...
        proofImage: proofBase64, // Store as Base64 string
        userId: userId,
        userName: userName,
      ...
      const localPost = {
        ...
        proof: proofBase64 // Store Base64 string directly
      };
