...
// Authentication check AND load posts - COMBINED into one useEffect
useEffect(() => {
  ...
  // Now load posts since user is authenticated
  console.log('Loading posts for session:', session);

  // IMPORTANT: Extract user ID EXACTLY as stored in database
  let userId = null;
  if (session?.user?.email) {
    // Use the same format as when creating posts
  ...
  const loadPosts = async () => {
    setLoading(true);
    try {
      console.log('Loading posts for exact userId:', userId);
      
      // Load posts with user filter
      ...
      if (fetchedPosts.length === 0) {
        console.log('No posts found for this user. Checking all posts...');
        
        // TEMPORARY: Fetch all posts to debug
        const allPosts = await postsDB.getPosts('requests', {});
        console.log('All posts in database:', allPosts);
        console.log('All post user IDs:', allPosts.map(p => ({ id: p.id, userId: p.userId })));
      }
      ...
      const transformedPosts = fetchedPosts.map(post => ({
        ...
        userPicture: post.userPicture || session?.user?.image || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=100&q=80',
...
}, [session, searchQuery, status, router]); // Added router to dependencies
  ...
  return (
    <div className="max-w-[1200px] mx-auto p-4 sm:p-5">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
       ...
       {/* DEBUG BUTTON - ADD THIS HERE */}
    <div className="mb-4">
      <Button 
        variant="secondary"
        onClick={async () => {
          // Test: Fetch all posts
          const all = await postsDB.getPosts('requests', {});
          console.log('All posts test:', all);
          
          // Test: Fetch with email-based userId
          const email = session?.user?.email;
          if (email) {
            const userId = email.replace(/[^a-zA-Z0-9]/g, '_');
            const userPosts = await postsDB.getPosts('requests', { userId });
            console.log('User posts test:', userPosts);
            
            // Also log the exact userId being used
