...
export default function SkillRequestPlatform() {
  ...
  useEffect(() => {
    ...
    if (session?.user?.email) {
      ...
      currentUserId = session.user.email.replace(/[^a-zA-Z0-9]/g, '_');
      console.log('Current userId from email:', currentUserId);
    }
    ...
    const loadPosts = async () => {
      ...
      try {
        ...
        // REMOVE userId filter - fetch ALL posts
        const fetchedPosts = await postsDB.getPosts('requests', {
          // userId: currentUserId, // REMOVE THIS LINE - fetch all posts
        ...
        const transformedPosts = fetchedPosts.map(post => ({
          ...
          // Add this flag to identify if post belongs to current user
    ...
    const unsubscribe = postsDB.listenToPosts('requests', {
      // userId: currentUserId, // REMOVE THIS LINE - listen to all posts
      searchQuery: searchQuery || ''
    }, (fetchedPosts) => {
  ...
  filteredPosts.map((post) => {
    ...
    return (
      <PostCard
        ...
        onDelete={() => handleDelete(post)} // Add this
        onEdit={() => handleEdit(post)} // Add this
        showProofImage={true}
        isOwnPost={isOwnPost} // Pass this flag
      />
    );
  })
) : (
