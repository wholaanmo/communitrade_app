...
export default function SkillRequestPlatform() {
  ...
  // Handle file upload to Firebase Storage - FIXED: Check for session.user structure
  const handleFileUpload = async (file) => {
    if (!file || !session) return null;
    
    try {
      // Get user ID from session - check different possible structures
      const userId = session.user?.id || session.user?.uid || session.userId || 'anonymous';
      
      // Create a safe filename
      const timestamp = Date.now();
      const safeFileName = file.name.replace(/[^a-zA-Z0-9.-]/g, '_');
      const storageRef = ref(storage, `proofs/${userId}/${timestamp}_${safeFileName}`);
  ...
  // Submit form to Firebase - FIXED: Check session structure
const submitForm = async (e) => {
  ...
  try {
    ...
    // Get user info from session - FIXED VERSION
    let userId = 'unknown';
    ...
    if (session?.user) {
      // NextAuth usually stores ID in token.sub, not in user object
      // Try to get ID from different places
      ...
      // Get user ID from session - FIXED: Check different possible structures
      const userId = session.user?.id || session.user?.uid || session.userId || 'anonymous';
      const userName = session.user?.name || session.user?.username || 'Current User';
      const userEmail = session.user?.email || 'No email';
      const userPicture = session.user?.image || session.user?.picture || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=100&q=80';

      // Prepare post data for Firebase - FIXED: Ensure all fields are defined
      const postData = {
        ...
        userId: userId, // FIXED: This was undefined
        userName: userName,
      ...
      if (!postData.title || !postData.description || !postData.category || !postData.location || !postData.schedule) {
      ...
      >
        <form onSubmit={submitForm} className="space-y-4">
          ...
          <div className="flex flex-col">
            <Label htmlFor="proof">Proof (Optional - Max 5MB, Image files only)</Label>
            <FileUpload
              id="proof"
            ...
            {formData.proof && (
              <small className="text-gray-500 mt-1">
                Selected: {formData.proof.name} ({(formData.proof.size / 1024 / 1024).toFixed(2)} MB)
              </small>
            )}
          </div>
