...
export default function SkillRequestPlatform() {
  ...
  useEffect(() => {
    ...
    // Fetch posts from Firebase when authenticated
    if (session) {
      fetchPosts();
    }
  ...
  const fetchPosts = async () => {
    try {
      ...
      // Transform Firebase data to match your PostCard component structure
      const formattedPosts = requests.map(post => ({
        id: post.id,
        ...
        skillsOffered: post.skillsOffered || post.skills || '',
  ...
  // Sample user profiles data (temporary - you might want to fetch this from Firebase too)
  const userProfiles = {
    'Alex Johnson': {
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      let proofUrl = null
      if (formData.proof) {
        proofUrl = await uploadImage(formData.proof)
      }
      ...
      const formattedPost = {
        ...
        skillsOffered: postData.skillsOffered,
  ...
  const handleContact = (post) => {
    ...
    alert(`Contact feature for ${post.userName} will be implemented soon!`)
  }
  ...
  const handleReportSubmit = async (reportData) => {
    try {
      ...
      await postsDB.reportPost('request', currentReportPost.id, reportInfo)
