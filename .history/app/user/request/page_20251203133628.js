...
export default function SkillRequestPlatform() {
  ...
  // =============== FIXED: Load posts from Firebase ===============
  useEffect(() => {
      ...
      try {
        ...
        const transformedPosts = fetchedPosts.map(post => {
          ...
          return {
            ...
            skillsOffered: Array.isArray(post.skillsOffered) 
              ? post.skillsOffered.join(', ') 
              : post.skillsOffered || '',
    ...
    if (session) {
      ...
      const unsubscribe = postsDB.listenToPosts('requests', {}, (fetchedPosts) => {
        if (!isMounted.current) return;
    ...
    // Cleanup for when session is null
    return () => {
      isMounted.current = false;
    };
  }, [session]);
  ...
  const formatTimeAgo = (timestamp) => {
    ...
    try {
      ...
      // Handle Firebase server timestamp placeholder
      else if (timestamp && timestamp['.sv'] === 'timestamp') {
        return 'Just now';
      }
      // Handle ISO string
      ...
      // Handle Unix timestamp (number)
        ...
        console.warn('Unhandled timestamp format:', timestamp);
        return 'Recently';
      }
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      const userId = session.user?.id || session.user?.email || `user_${Date.now()}`;
      const userName = session.user?.name || 'Current User';
      const userPicture = session.user?.image || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=100&q=80';
      ...
      // Save to Firebase - Use 'request' (singular) as type
  ...
  return (
    <div className="max-w-[1200px] mx-auto p-4 sm:p-5">
      ...
      <div className="mb-4 text-sm text-gray-600">
        Showing {posts.length} posts (Filtered: {filteredPosts.length})
      </div>
