  ...
  return (
    <div className="relative">
      <PostCard
        ...
        // Add some padding at the bottom to make room for buttons
        className="pb-16" // Add this line
      />
      <div className="absolute bottom-6 right-6 flex gap-2">
        {isOwnPost ? (
          ...
          <>
            <RequestActionButton type="report" onClick={() => onReport(post)} className="text-xs">
              Report
            </RequestActionButton>
            <RequestActionButton type="contact" onClick={() => onContact(post)} className="text-xs">
              Contact
            </RequestActionButton>
            <RequestActionButton type="profile" onClick={() => onViewProfile(post)} className="text-xs">
              View Profile
            </RequestActionButton>
          </>
        )}
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      if (editingPostId) {
        ...
        // Create new post
        const newPost = await postsDB.createPost('requests', postData);
