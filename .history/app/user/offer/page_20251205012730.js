...
const OfferCardWithButtons = ({ 
  ...
  return (
    <PostCard
      post={post}
      {...props}
      // Use custom actions instead of default ones
      customActions={true}
...
export default function SkillOfferPlatform() {
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      ...
      try {
        ...
        // Fetch ALL posts (no userId filter) - use 'offers' collection
    ...
    // Add real-time listener for ALL offer posts
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      let userPicture = 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?...';
      ...
      if (editingPostId) {
        ...
        updatedPosts = posts.map(post =>
          post.id === editingPostId
            ? { ...post, ...postData, timePosted: 'Updated just now' }
            : post
        );
        ...
        // ✅ CREATE NEW POST
        const newPost = await postsDB.createPost('offers', postData);
