...
export default function UsersManagement() {
  ...
  return (
    <div className="users-management-page p-8 max-w-7xl mx-auto">
        ...
        {filteredUsers.map(user => (
          <div key={user.id} className="user-profile-container bg-white rounded-xl p-6 shadow-sm border border-gray-300 transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
            <div className="user-main-info flex justify-between items-start mb-4">
              <div className="name-verified flex items-center gap-2 flex-wrap">
              ...
              <button 
                className="view-records-btn bg-gradient-to-br from-[#121731] to-[#728a9c] text-white border-none px-4 py-2 rounded-lg cursor-pointer text-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
      ...
      {/* Profile Modal */}
      {showProfileModal && (
        <div className="modal-overlay fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50 backdrop-blur-sm" onClick={closeProfileModal}>
          <div className="modal-content profile-modal bg-white rounded-2xl w-[90%] max-h-[90vh] overflow-y-auto shadow-2xl max-w-2xl p-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="profile-actions flex justify-between mt-8 pt-5 border-t border-gray-300">
              <button 
                className={`btn px-5 py-3 rounded-lg cursor-pointer transition-colors duration-300 ${currentProfile.isDeactivated ? 'reactivate-btn bg-green-600 hover:bg-green-700' : 'deactivate-btn bg-red-600 hover:bg-red-700'} text-white border-none`}
      ...
      {showDeactivateModal && (
        <div className="modal-overlay fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50 backdrop-blur-sm" onClick={closeDeactivateModal}>
      ...
      {showDeleteModal && (
        <div className="modal-overlay fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50 backdrop-blur-sm" onClick={closeDeleteModal}>
