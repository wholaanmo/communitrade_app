...
export default function SkillRequestPlatform() {
  ...
  const [showKeepModal, setShowKeepModal] = useState(false); // New state for keep confirmation
  ...
  const [postToKeep, setPostToKeep] = useState(null); // New state for post to keep
  ...
  // New function to open keep confirmation modal
  const openKeepModal = (post) => {
    setPostToKeep(post);
    setShowKeepModal(true);
  };

  // New function to close keep confirmation modal
  const closeKeepModal = () => {
    setShowKeepModal(false);
    setPostToKeep(null);
  };

  // Updated keepPost function to use confirmation
  const confirmKeep = () => {
    if (postToKeep) {
  ...
  // New function to handle delete from profile modal
  ...
  return (
    <div className="skill-request-platform max-w-[1200px] mx-auto p-5">
      ...
      <div className="search-report-container flex items-center gap-5 mb-8">
        ...
        <button 
          className={`reported-posts-btn flex items-center text-white px-5 py-3 rounded-lg font-medium transition-colors duration-300 whitespace-nowrap ${
            showReportedPosts ? 'bg-[#728a9c] hover:bg-[#5a7284] request-posts-btn' : 'bg-[#e74c3c] hover:bg-[#c0392b]'
          }`}
      ...
      <div className="header-container flex justify-between items-center mb-8">
        <h1 className="requests-header text-[#121731] text-2xl font-semibold">
          {showReportedPosts ? 'Reported Posts' : 'Requests'}
        </h1>
        <div className="request-count bg-gray-100 px-4 py-2 rounded-lg border border-gray-300 font-medium">
          {showReportedPosts ? 'Total Reported:' : 'Total Requests:'} <span className="font-bold text-[#121731]">
          ...
          {filteredPosts.map(post => (
            <div key={post.id} className="post-card bg-white rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
              ...
              <div className="post-content p-5">
                <h2 className="request-title m-0 mb-4 text-xl text-[#121731]">{post.request}</h2>
  ...
  >
    Cancel
      ...
      {showKeepModal && (
        <div 
        className="modal-overlay keep-overlay fixed inset-0 bg-[rgba(211,211,211,0.5)] backdrop-blur-[1px] flex justify-center items-center p-5 z-[2000]" 
        onClick={closeKeepModal}
      >      
          <div className="modal-content keep-modal bg-white rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl p-8" onClick={(e) => e.stopPropagation()}>
            <div className="keep-header mb-6 text-center">
              <h2 className="m-0 text-[#121731] text-2xl">Keep Reported Post</h2>
            </div>
            
            <div className="keep-content mb-8 text-center">
              <div className="keep-warning-icon mb-4">
                <span className="material-icons text-green-600 text-5xl">help</span>
              </div>
              <p className="keep-warning text-xl font-semibold text-gray-900 mb-4">
                Are you sure you want to keep this reported post?
              </p>
              <p className="keep-info text-[#728a9c] leading-relaxed mb-0">
                This post will be removed from the reported list but will remain visible on the platform.
              </p>
            </div>
            
            <div className="keep-actions flex gap-4 justify-center">
              <button 
                className="btn confirm-keep-btn px-4 py-2 bg-green-600 text-white border-none rounded-lg cursor-pointer transition-colors duration-300 hover:bg-green-700" 
                onClick={confirmKeep}
              >
                Confirm Keep
              </button>
                  ...
                  {currentProfile.skillOffers?.map(offer => (
                    <div key={offer.id} className="profile-post-card bg-gray-50 rounded-lg p-5 border border-gray-300">
                      ...
                      <div className="post-actions flex gap-3 mt-4">
                        <button 
                          className="btn delete-btn flex items-center px-3 py-1.5 rounded-lg font-medium transition-all duration-300 text-red-600 border border-red-600 hover:bg-red-600 hover:text-white text-sm"
