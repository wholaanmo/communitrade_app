...
import { adminDB, statsDB } from '@/lib/firebaseDB';
...
export default function AdminDashboard() {
    ...
    const fetchAdminData = async () => {
      try {
        ...
        setRemovedPosts(reported.map(post => ({
          ...
          userProfilePicture: post.userPicture || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=100&q=80'
        })));

        // For kept posts, we'll show active posts that haven't been reported
        // In a real app, you would have a separate flag for kept posts
        ...
        const kept = allPosts
          ...
          .map(post => ({
            ...
            type: post.type || 'offer',
            skills: post.skills || 'Unknown Skill',
            request: post.title || post.request || 'Unknown Request',
            description: post.description || 'No description',
            category: post.category || 'General',
            location: post.location || 'Not specified',
            skillsRequested: post.skillsRequested || 'Not specified',
            ...
            schedule: post.schedule || 'Flexible',
            additionalNotes: post.additionalNotes || '',
            reportReason: 'Verified as legitimate post',
            userId: post.userId,
            userFullName: post.userName || post.userEmail?.split('@')[0] || 'Unknown User',
            userProfilePicture: post.userPicture || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=100&q=80'
          }));
  ...
  const reactivateAccount = async (accountId) => {
    try {
      await adminDB.reactivateUser(accountId);
      // Remove from local state
      setDeactivatedAccounts(deactivatedAccounts.filter(account => account.id !== accountId));
      setStats(prev => ({ ...prev, deactivatedAccounts: prev.deactivatedAccounts - 1 }));
    ...
    try {
      ...
      if (user) {
        setCurrentProfile({
          ...
          userPicture: user.image || user.profilePicture || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=100&q=80',
          isVerified: user.emailVerified || false,
          linkedAccounts: user.linkedAccounts || [],
          skillOffers: user.offers || [],
          skillRequests: user.requests || []
        });
  ...
  const updateUserRole = async (userId, newRole) => {
    try {
      await adminDB.updateUserRole(userId, newRole);
    ...
    <div className="admin-dashboard max-w-6xl mx-auto p-4 sm:p-5 lg:p-6">
      ...
      <div className="stats-container grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div className="stat-card deactivated-stat bg-white rounded-xl p-4 sm:p-6 shadow-lg transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl">
          ...
          <div className="stat-count text-4xl sm:text-5xl font-bold text-center text-[#e74c3c]">{stats.deactivatedAccounts}</div>
        </div>
        ...
        <div className="stat-card removed-stat bg-white rounded-xl p-4 sm:p-6 shadow-lg transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl">
          ...
          <div className="stat-count text-4xl sm:text-5xl font-bold text-center text-[#e74c3c]">{stats.removedPosts}</div>
        </div>
        ...
        <div className="stat-card kept-stat bg-white rounded-xl p-4 sm:p-6 shadow-lg transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl">
          ...
          <div className="stat-count text-4xl sm:text-5xl font-bold text-center text-[#27ae60]">{stats.keptPosts}</div>
        </div>
      </div>
                  ...
                  <div key={account.id} className="account-item bg-gray-50 rounded-lg p-4 sm:p-5 mb-3 sm:mb-4 border border-gray-200 relative">
                    <div className="account-header flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                      ...
                      <div className="account-info flex-1 text-center sm:text-left">
                        ...
                        {account.reason && (
                          <p className="deactivation-reason text-xs text-gray-600 mt-2">Reason: {account.reason}</p>
                        )}
                      </div>
                      <button 
            ...
            <div className="profile-sections flex flex-col gap-4 sm:gap-6">
              ...
              <div className="profile-section">
                ...
                <div className="linked-accounts flex flex-col gap-2 sm:gap-2.5">
                  {currentProfile.linkedAccounts?.length > 0 ? (
              ...
              <div className="profile-section">
                ...
                <div className="posts-list flex flex-col gap-4 sm:gap-5">
                  {currentProfile.skillOffers?.length > 0 ? (
                    ...
                    <p className="text-gray-500 text-sm">No skill offer posts</p>
                  )}
                </div>
              </div>
              ...
              <div className="profile-section">
                ...
                <div className="posts-list flex flex-col gap-4 sm:gap-5">
                  {currentProfile.skillRequests?.length > 0 ? (
                    ...
                    <p className="text-gray-500 text-sm">No skill request posts</p>
                  )}
                </div>
              </div>
            </div>
