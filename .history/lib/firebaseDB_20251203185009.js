...
import { 
  ...
  export const postsDB = {
    ...
    async getPosts(type, filters = {}) {
      console.log(`Getting posts from: ${type}`);  // Remove the 's'
    ...
    listenToPosts(type, filters = {}, callback) {
      ...
      if (filters.userId) {
        console.log(`Real-time filter by userId: ${filters.userId}`);
        postsQuery = query(postsRef, orderByChild('userId'), equalTo(filters.userId));
      }
      
      const unsubscribe = onValue(postsQuery, (snapshot) => {
        console.log(`Real-time snapshot for ${type}s:`, snapshot.size, 'posts');
