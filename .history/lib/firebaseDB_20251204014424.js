...
import { 
  ...
  export const messagingDB = {
    ...
    async getOrCreateConversation(currentUserId, otherUserId, currentUserData = {}, otherUserData = {}) {
      try {
        ...
        let existingConversationKey = null;
        ...
        if (snapshot.exists()) {
          snapshot.forEach((childSnapshot) => {
            ...
            if ((participants.user1 === currentUserId && participants.user2 === otherUserId) ||
              ...
              existingConversationKey = childSnapshot.key; // Get the Firebase key
            }
          });
        }
        
        if (existingConversation) {
          console.log('Found existing conversation:', existingConversationKey);
          return {
            ...existingConversation,
            id: existingConversationKey, // Make sure the id is the Firebase key
            firebaseId: existingConversationKey // Add firebaseId for clarity
          };
        }
  ...
  async getUserConversations(userId) {
    try {
      ...
      const seenConversationIds = new Set(); // Track unique conversation IDs
        ...
        snapshot.forEach((childSnapshot) => {
          ...
          if (participants.user1 === userId || participants.user2 === userId) {
            ...
            if (seenConversationIds.has(conversationId)) {
              console.warn(`Duplicate conversation ID found: ${conversationId}, skipping...`);
              return;
            }
            
            seenConversationIds.add(conversationId);
            
            // Clean up duplicate ID from conversation data
            const cleanConversation = { ...conversation };
            if (cleanConversation.id === conversationId) {
              delete cleanConversation.id;
            }
  ...
  listenToUserConversations(userId, callback) {
    ...
    const unsubscribe = onValue(conversationsRef, (snapshot) => {
      ...
      console.log(`Returning ${userConversations.length} unique conversations for user: ${userId}`);
  ...
  async markMessagesAsRead(conversationId, userId) {
    try {
      ...
      // REMOVED the unreadCount update to avoid the error
      // You can implement unread count differently if needed
