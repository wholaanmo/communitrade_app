...
const ImageZoomModal = ({ isOpen, onClose, imageUrl, altText = "Proof image" }) => {
  ...
  const handleZoomIn = () => {
    ...
    if (img) {
      ...
      img.style.transform = `scale(${Math.min(currentScale * 1.2, 3)})`
    }
  }
    ...
    >
      <div className="relative w-full h-full flex items-center justify-center bg-black bg-opacity-90">
        ...
        <Button
          variant="secondary"
          onClick={onClose}
          className="absolute top-4 right-4 z-50 bg-white bg-opacity-20 hover:bg-opacity-30 text-white border-none"
          size="sm"
        >
