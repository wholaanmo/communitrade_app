...
export default function ProfileModalAdmin({
  ...
  const isValidImageUrl = (url) => {
    if (!url) return false
    // Check if it's a data URL or a proper http URL
              ...
              {skillOffers.length > 0 ? (
                skillOffers.map(offer => (
                  <Card key={offer.id} className="p-5 border border-gray-300">
                    ...
                    {offer.proof && isValidImageUrl(offer.proof) && (
                      <div className="proof-image my-4">
                        <div className="flex flex-col">
                          ...
                          <img 
                            src={offer.proof} 
                            alt="Proof" 
                            className="proof-thumbnail w-full max-w-[200px] rounded-lg border border-gray-300 shadow-sm"
                            onError={handleImageError}
                            onLoad={(e) => console.log('Image loaded successfully:', offer.proof)}
                          />
                        </div>
                      </div>
                    )}
                    {offer.proof && !isValidImageUrl(offer.proof) && (
                      <div className="proof-placeholder my-4">
                        <SmallText color="gray" className="italic">Proof image unavailable</SmallText>
                      </div>
                    )}
              ...
              {skillRequests.length > 0 ? (
                skillRequests.map(request => (
                  <Card key={request.id} className="p-5 border border-gray-300">
                    ...
                    {request.proof && isValidImageUrl(request.proof) && (
                      <div className="proof-image my-4">
                        <div className="flex flex-col">
                          <SmallText weight="semibold" color="dark" className="mb-2">Proof:</SmallText>
                          <img 
                            ...
                            className="proof-thumbnail w-full max-w-[200px] rounded-lg border border-gray-300 shadow-sm"
                            onError={handleImageError}
                            onLoad={(e) => console.log('Image loaded successfully:', request.proof)}
                          />
                        </div>
                      </div>
                    )}
