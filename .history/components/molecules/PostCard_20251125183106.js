...
import ImageZoomModal from './ImageZoomModal' // Import the ImageZoomModal
...
export default function PostCard({
  ...
  customActions = false,
  showProofImage = false // Add this new prop
}) {
  const { userName, userPicture, timePosted, request, description, category, location, skills, proof, additionalNotes } = post
  
  const [showZoomModal, setShowZoomModal] = useState(false) // State for zoom modal
  ...
  // Function to handle proof image click
  const handleProofImageClick = (imageUrl) => {
    ...
    <>
      <Card hover className={`min-h-[580px] flex flex-col ${className}`}>
        ...
        <div className="mb-6 flex-1">
          ...
          {/* Proof Images - Made Larger with Zoom Feature */}
          {showProofImage && proof && (
            <div className="mt-4">
              <SmallText weight="semibold" color="dark" className="mb-3">Proof:</SmallText>
              <div className="flex flex-wrap gap-3">
                {Array.isArray(proof) ? (
                  proof.map((proofItem, index) => {
                    const imageUrl = typeof proofItem === 'string' ? proofItem : URL.createObjectURL(proofItem)
                    return (
                      <div 
                        key={index}
                        className="relative group cursor-zoom-in"
                        onClick={() => handleProofImageClick(imageUrl)}
                      >
                        <img
                          src={imageUrl}
                          alt={`Proof ${index + 1}`}
                          className="w-40 h-40 object-cover rounded-lg border-2 border-gray-300 shadow-sm hover:shadow-md transition-all duration-300 group-hover:opacity-90"
                          onError={handleProofImageError}
                        />
                        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-10 transition-all duration-300 rounded-lg flex items-center justify-center">
                          <Icon name="zoom_in" className="text-white text-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        </div>
                      </div>
                    )
                  })
                  ...
                  <div 
                    className="relative group cursor-zoom-in"
                    onClick={() => handleProofImageClick(typeof proof === 'string' ? proof : URL.createObjectURL(proof))}
                  >
                    <img
                      src={typeof proof === 'string' ? proof : URL.createObjectURL(proof)}
                      alt="Proof"
                      className="w-48 h-48 object-cover rounded-lg border-2 border-gray-300 shadow-sm hover:shadow-md transition-all duration-300 group-hover:opacity-90"
                      onError={handleProofImageError}
                    />
                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-10 transition-all duration-300 rounded-lg flex items-center justify-center">
                      <Icon name="zoom_in" className="text-white text-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <ImageZoomModal
        ...
        altText={`Proof for ${post.request}`}
      />
    </>
  )
}